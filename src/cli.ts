...
const routeRegistrySource = (files: readonly string[], routesDir: string, registryFile: string): string => {
  const pages = pagesFromRouteFiles(files, { rootDir: routesDir }).sort((left, right) =>
  ...
  const entries = pages.map(
    (page, index) =>
      `  { assetPrefix: ${JSON.stringify(page.assetPrefix)}, fileName: ${JSON.stringify(page.fileName)}, path: ${JSON.stringify(page.path)}, template: routeSource${index} },`,
  );
...
export const addPageFiles = async (options: Omit<CliAddPageOptions, "command">): Promise<Result<string, string>> => {
  ...
  if (!options.force && (overwriting || overwritingDeclarations)) {
    const conflicts = [
      overwriting ? pageFile : undefined,
      overwritingDeclarations ? declarationFile : undefined,
  ...
  const routeFiles = existingRoutes
    .filter((route) => route.kind === "template" && route.file !== pageFile)
