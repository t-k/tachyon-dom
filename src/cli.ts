...
const generatedRoutesMarker = "// Generated by tachyon-dom. Do not edit by hand.";

const routeRegistrySource = (files: readonly string[], routesDir: string, registryFile: string): string => {
  const pages = pagesFromRouteFiles(files, { rootDir: routesDir }).sort((left, right) => left.path.localeCompare(right.path));
  const imports = pages.map((page, index) => {
    const specifier = relative(dirname(registryFile), page.file).replaceAll("\\", "/");
    return `import routeSource${index} from ${JSON.stringify(`${specifier.startsWith(".") ? specifier : `./${specifier}`}?raw`)};`;
  });
  const entries = pages.map(
    (page, index) => `  { assetPrefix: ${JSON.stringify(page.assetPrefix)}, fileName: ${JSON.stringify(page.fileName)}, path: ${JSON.stringify(page.path)}, template: routeSource${index} },`,
  );
  return `${generatedRoutesMarker}\n${imports.join("\n")}\n\nexport const pages = [\n${entries.join("\n")}\n] as const;\n`;
};

const pageSource = (title: string): string => `<script lang="ts">
export const scope = () => ({
...
export const addPageFiles = async (options: Omit<CliAddPageOptions, "command">): Promise<Result<string, string>> => {
  ...
  if (!normalized || normalized.includes("\\") || segments.some((segment) => !segment || segment === "." || segment === "..")) {
  ...
  const declarationFile = `${pageFile}.d.ts`;
  const registryFile = join(dirname(options.routesDir), "routes.generated.ts");
  let overwriting = false;
  let overwritingDeclarations = false;
  ...
  try {
    ...
    overwritingRegistry = true;
    if (!registry.startsWith(generatedRoutesMarker) && !options.force) {
      return err(`Refusing to replace non-generated route registry: ${registryFile}`);
    }
  } catch (error) {
  ...
  let existingRoutes: Awaited<ReturnType<typeof scanFileRoutes>> = [];
  try {
    existingRoutes = await scanFileRoutes(options.routesDir);
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
  }
  const routeFiles = existingRoutes.filter((route) => route.kind === "template" && route.file !== pageFile).map((route) => route.file);
  routeFiles.push(pageFile);
  await mkdir(targetDir, { recursive: true });
  await writeFile(pageFile, source);
  await writeFile(declarationFile, declarations.value);
  await writeFile(registryFile, routeRegistrySource(routeFiles, options.routesDir, registryFile));
  return ok(
    [
      overwriting ? `Overwrote ${pageFile}.` : `Created ${pageFile}.`,
      `Route URL: ${routeUrl}`,
      "Edit the generated page.td to define the route markup and scope.",
      overwritingDeclarations ? `Overwrote ${declarationFile}.` : `Generated declarations: ${declarationFile}`,
      overwritingRegistry ? `Updated route registry: ${registryFile}` : `Generated route registry: ${registryFile}`,
    ].join("\n"),
  );
};
...
const starterPageSource = (): string => `<script lang="ts">
export const scope = () => ({
...
import { pages } from "./routes.generated";
...
const starterServerSource = (): string => `import { renderAppResponse } from "tachyon-dom/app";
import { app } from "./app";

export const render = (path = "/") => renderAppResponse(app, path, { minify: true });
`;

const starterSmokeTestSource = (): string => `import { describe, expect, it } from "vitest";
import { app } from "./app";

describe("starter app", () => {
  it("renders the starter page", () => {
    expect(app.renderRoute("/")).toContain("<h1>Welcome</h1>");
  });
});
`;
...
const starterGitignoreSource = (): string => `node_modules/
dist/
.vite/
coverage/
`;

const starterCiSource = (): string => `name: CI
...
\`src/routes.generated.ts\` is the generated route registry used by the app, Vite, tests, and SSR. \`tachyon-dom add page\` updates it automatically. Do not edit it by hand. Dynamic directories use \`[id]\` and catch-all directories use \`[...slug]\` across the app and server route manifest.

The Vite plugin refreshes adjacent \`.td.d.ts\` declarations during development and builds. The starter and \`add page\` create the initial declaration, so \`pnpm typecheck\` works before the first Vite run. Do not run per-file type generation manually.
...
Template: \`${template}\`.
${template === "ssr" ? "\n## SSR entry\n\nThe `ssr` template includes `src/server.ts` with a small `render(path)` helper for server-side composition.\n" : ""}
`;
...
export const createStarterFiles = async (options: Omit<CliInitOptions, "command">): Promise<Result<string, string>> => {
  ...
  const starterPageFile = join(options.outDir, "src", "routes", "index", "page.td");
  const starterRegistryFile = join(options.outDir, "src", "routes.generated.ts");
  const starterSource = starterPageSource();
  const starterDeclarations = generateTachyonModuleTypes(starterSource);
  if (!starterDeclarations.ok) return starterDeclarations;
  await writeFile(starterPageFile, starterSource);
  await writeFile(`${starterPageFile}.d.ts`, starterDeclarations.value);
  await writeFile(
    starterRegistryFile,
    routeRegistrySource([starterPageFile], join(options.outDir, "src", "routes"), starterRegistryFile),
  );
  ...
  await writeFile(join(options.outDir, "src", "app.test.ts"), starterSmokeTestSource());
  if (options.template === "ssr") {
    await writeFile(join(options.outDir, "src", "server.ts"), starterServerSource());
  }
  ...
  await writeFile(join(options.outDir, ".gitignore"), starterGitignoreSource());
