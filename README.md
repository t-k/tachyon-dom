...
Production app documents use the safe `whitespace: "condense"` policy by default. This policy only normalizes whitespace inside HTML tag syntax; it preserves every text node and comment byte, including hydration boundaries, text-binding separators, inline word boundaries, and `pre`, `textarea`, `script`, and `style` content. Use `app.renderDocument(path, { whitespace: "preserve" })` to retain tag formatting too. The older `minify: true` app option and `minifyHtml` Vite option remain compatibility aliases; new configurations should use the typed whitespace policy.

Buffered route rendering can opt in with `renderRoute(routes, request, { htmlWhitespace: "condense" })`, and Node, Workers, and Lambda route handlers accept the same option. Buffered dynamic rendering defaults to `"preserve"`. Streaming always preserves each chunk and never buffers a response for HTML condensation, so early flush, cancellation, and backpressure behavior are unchanged.
