...
`,
  ...
  it("verifies package export and bin artifacts before release", async () => {
    ...
    expect(packageJson.scripts?.["verify:package"]).toBe(
      "node scripts/verify-package-artifacts.mjs && node scripts/verify-middleware-context-types.mjs && node scripts/verify-client-route-types.mjs",
    );
    expect(packageJson.scripts?.["verify:client-route-types"]).toBe("node scripts/verify-client-route-types.mjs");
