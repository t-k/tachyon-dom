...
- Measured client sizes grew with the memo state and the insertion-aware walkers: the browser entry (`createSignal` only) is 1037 bytes minified, the quick example 16382 bytes minified and 5657 bytes Brotli, `runtime/list` 15432 bytes minified. Budgets and baselines follow the measured sizes.
