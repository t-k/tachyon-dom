...
- `listRegionStartBetween()` honours its end bound inside an insertion: when the bound sits on or inside a `<slot>` or `<outlet>` region, the lookup returns nothing instead of skipping past the bound to a list after it.
...
- Measured client sizes grew with the memo state and the insertion-aware walkers: the browser entry (`createSignal` only) is 1237 bytes minified, the quick example 16719 bytes minified and 5773 bytes Brotli, `runtime/list` 15744 bytes minified. Budgets and baselines follow the measured sizes.
