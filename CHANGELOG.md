...
- Measured client sizes grew with the memo state and the insertion-aware walkers: the browser entry (`createSignal` only) is 1237 bytes minified, the quick example 16692 bytes minified and 5772 bytes Brotli, `runtime/list` 15744 bytes minified. Budgets and baselines follow the measured sizes.
