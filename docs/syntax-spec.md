...
| Markers       | Every target delimits the insertion with the same pair: `<!--tachyon-outlet-->`/`<!--/tachyon-outlet-->` or `<!--tachyon-slot:name-->`/`<!--/tachyon-slot:name-->`. The start marker occupies one logical child slot, the inserted nodes and the end marker occupy none, so bindings after the insertion keep their template paths however many nodes were inserted. No node lookup or region enumeration of the parent template counts anything inside the pair as the parent's, including `<for>` and `<if>` markers the inserted HTML may carry. The markers are generated-only (see [Public API layers](api.md)). |
