...
The client compiler records static element siblings around a direct `<for>` so SSR rows can be adopted without consuming those siblings. Bindings before the list keep their logical paths, while bindings after the list account for the expanded row region. A direct `<for>` can be hydrated when it is the sole dynamic region under its parent. This rule is checked recursively inside list row templates as well. If a `<for>` shares its parent with another direct `<for>` or `<if>`, `hydrate()` reports an ambiguous dynamic-region error before binding and leaves the SSR DOM untouched. Place the dynamic regions under separate parent elements when they must be hydrated together. Conditional branches with the same client shape as a static or conditional sibling are also rejected before binding because SSR output cannot identify their ownership.
