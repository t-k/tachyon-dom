...
| --- | ---: | ---: |
| runtime/list | 15744 | 5682 |
| runtime/generic-list | 20909 | 7185 |
| runtime/form | 2997 | 1210 |
| runtime/conditional | 9965 | 3494 |
| runtime/generic-conditional | 15967 | 5352 |
...
`createMemo()` exposes a cached computed accessor. Reading a memo always returns the value for the current signal state: inside `batch()` or an effect run, a memo whose recomputation is queued recomputes on read, and the memos it reads recompute first, so a value is derived from fresh dependencies and no memo observes a stale one. Reading a memo never runs an unrelated queued memo. A memo queued only because a memo it reads was queued is checked rather than recomputed: it pulls the memos it reads and runs its own callback only if one of them changed value or failure state, so an upstream memo that recomputes to an equal value stops the chain there, and neither downstream memos nor the effects behind them run. Ordinary effects still wait for the flush, so a batch sees fresh derived values and deferred side effects. Each memo recomputes at most once per flush. A memo holds either its last computed value or the failure of its last computation: a failure is delivered to the nearest reactive error owner, and every read of the failed memo rethrows it, including reads from a dependent memo or effect, until a dependency changes and the recomputation succeeds. The last successful value is never returned in place of a failure; a "keep the last good value" contract belongs to `createResource()`, whose `data` and `error` are separate accessors.
...
A generated module that combines an input scope with template-local state uses the merged scope store from the same subpath. Reads never write to the local store: they subscribe to both the input scope and the local key, and return a local value only while the input still holds the value it held when the template assigned. When the input changes, the input wins again and the local override is dropped. A write is therefore compared against the value the binding currently sees, not against a stale override, so an assignment that changes the visible value notifies its subscribers. Reading a key the scope does not have does not add it to `Object.keys()` or `in`, and inspecting a property descriptor does not evaluate an input getter. This is what keeps a getter that returns a fresh array or object from re-running every binding that reads it.
