...
The current `0.2.0-rc.1` manifest is preparation only. No tag or publication is implied by this version change. Keep the candidate heading marked Unreleased until release approval, then use `next` for the candidate and `latest` for the approved stable version. Do not publish a tag from a different SHA than the one verified by the shared workflow.
