...
Direct `name=${value}` interpolation is quoted automatically. Quote the complete attribute value when a template contains a prefix or suffix, such as `src="/assets/${file}.png"`; interpolation into an unquoted value is rejected. `rawHtml()` is accepted only in text context and must return to text context within that fragment; it cannot carry a partial tag, attribute, comment, or raw-text token into adjacent template content. The completed HTML is parsed once more so dangerous attributes and URL policies cannot be bypassed across fragment boundaries. Fragments returned by `attr()` and `booleanAttr()` are accepted only between attributes inside an opening tag.
...
HTML escaping does not make an active URL scheme safe. The compiler, client attribute runtime, direct server HTML helper, and generated server and stream renderers apply one URL policy to `href`, `src`, `action`, `formaction`, `xlink:href`, `object[data]`, `video[poster]`, `audio[poster]`, and `img` or `source` `srcset`. They reject `javascript:`, `vbscript:`, every `data:` URL, control-obfuscated schemes, protocol-relative references, and raw or percent-decoded backslashes. An invalid percent escape such as `%zz` is not itself unsafe: raw checks always run, and decoded checks run only when decoding succeeds. Direct `html` URL interpolation must supply the complete attribute value; build prefixes and suffixes before interpolation so the final URL can be validated as one value.

`srcset` accepts a comma-separated list with at most one positive `w` or `x` descriptor per candidate, and validates every candidate URL. Literal `data:` candidates are rejected, including image data URLs. A `meta` element whose `http-equiv` is statically `refresh` validates the complete `content` value and its optional URL; a dynamic refresh mode combined with `content` is rejected because the element policy cannot be resolved safely. Route head descriptors use the same whole-element policy on the server and during client navigation. Unsafe `link` URLs drop the element, while unsafe URL attributes on `meta` and `script` are omitted.
