...
Buffered routes and their Node, Workers, and Lambda handlers accept `htmlWhitespace: "normalize-tags"`. It normalizes tag-syntax whitespace only and preserves text nodes, comments, Tachyon hydration markers, and raw-text content. Use `tachyonDom({ templateWhitespace: "condense" })` to reduce formatting newlines inside compiled `.td` route templates consistently across client, server, and stream targets. `HtmlWhitespacePolicy` uses `"preserve-tags" | "normalize-tags"`, while compiler `TemplateWhitespacePolicy` uses `"preserve" | "condense"`, preventing variables of those named policy types from being passed to the wrong API. Deprecated `LegacyHtmlWhitespacePolicy` inputs keep the old `"preserve"` and `"condense"` values source-compatible and map them explicitly to tag preservation and tag normalization. Deprecated `minify`, `minifyHtml`, `minifyHtml()`, and `condenseHtmlWhitespace()` aliases also continue to select tag normalization only. Route streaming already waits for authoritative metadata and final route rendering; `htmlWhitespace` does not add another buffering pass or alter lower-level backpressure and cancellation.
