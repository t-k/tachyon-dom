...
`renderRouteStream()` resolves commit-critical route metadata before it returns. A route `fallback` is never sent while a loader can still change the status, redirect location, cache policy, cookies, or security headers. Use compiler-generated async stream fragments when progressive body chunks are required after the HTTP metadata has been committed. When only the deepest route defines `stream()`, ancestor `render()` functions still run once and their ordinary `${outlet}` position becomes a single, demand-driven stream slot. A legacy progressive ancestor must preserve that outlet exactly once. Use `streamLayout()` to return bounded `{ before, after, outlet: "once" | "omit" }` segments when the layout intentionally omits the child or cannot preserve the opaque compatibility outlet. Multiple stream outlets are not supported because replay would require buffering the child body.

Direct `renderRoute()` and `renderRouteStream()` calls keep `headHtml`, `resourceHints`, and `stateScript` as separate fields. Route adapters compose those fields by default: the fragment document places head metadata before the route fragment and hydration state after it. This preserves body bytes when all metadata is empty, but it does not inspect arbitrary HTML or search for closing tags. For a semantically complete document, keep route renders as body fragments and configure the adapter with `document: htmlDocument({ lang: "en", head: '<meta charset="utf-8">' })`. The helper owns the doctype, `html`, `head`, and `body`, places route metadata in `head`, and places hydration state at the end of `body`. The optional static `head`, `bodyBefore`, and `bodyAfter` strings are trusted application HTML.
...
Single files can also use `/// <reference types="tachyon-dom/td-modules" />`. The entry declares `.td`, `.td?client`, `.td?client&mount-only`, `.td?client&hydrate-only`, `.td?server`, `.td?stream`, `.td?entry`, and `.td?raw` modules with the Vite plugin output types, and the same set for the `.tachyon` and `.tachyon.html` extensions. The plugin accepts all three extensions for every mode, so a template keeps its types when only its extension differs.
